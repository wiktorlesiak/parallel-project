...
// count number of Human neighbors in Moore neighborhood
int humanNeighborCount(int **world, int x, int y) {
   ...
   //printf("Human Value of NorthWest neighbor [1] is: %d\n", neighbor1);
    // North Neighbor
    neighbor2 = world[(((i-1)+size)%size)][j];
    // printf("Human Value of North neighbor (2) is: %d\n", neighbor2);
    // NorthEast Neighbor
    neighbor3 = world[(((i-1)+size)%size)][(j+1)%size];
    // printf("Human Value of NorthEast neighbor (3) is: %d\n", neighbor3);
    // West Neighbor
    neighbor4 = world[(i)][(((j-1)+size)%size)];
    // printf("Human Value of West neighbor (4) is: %d\n", neighbor4);
    // East Neighbor
    neighbor5 = world[(i)][(j+1)%size];
    //printf("Human Value of East neighbor (5) is: %d\n", neighbor5);
    // SouthWest Neighbor
    neighbor6 = world[((i+1)%size)][(((j-1)+size)%size)];
    //printf("Human Value of SouthWest neighbor (6) is: %d\n", neighbor6);
    // South Neighbor
    neighbor7 = world[((i+1)%size)][j];
    //printf("Human Value of South neighbor (7) is: %d\n", neighbor7);
    // SouthEast Neighbor
    neighbor8 = world[((i+1)%size)][(j+1)%size];
    //printf("Human Value of SouthEast neighbor (8) is: %d\n", neighbor8);
    ...
    //printf("Function: Human Neighbors of Cell[%d][%d] is: %d\n", i,j, humanCounter);
